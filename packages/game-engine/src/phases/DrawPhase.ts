...
export class DrawPhase extends Phase {
  ...
  async execute(context: any): Promise<any> {
    ...
    // 布道 (张三丰): 摸牌阶段可摸3张, 然后将其中1张交给任意一名角色 (含自己)
    // 不受画地为牢影响 (画地为牢跳过出牌阶段, 摸牌照常进行, 同理布道生效)
    // 受南蛮/天命生效后跳过摸牌的影响: 跳到此处时 skipDrawThisTurn 已被设置
    if (player.hasSkillOrTreasure('bu-dao')) {
      // 询问是否发动 (玩家专用); AI 默认发动 (3张优于2张)
      let trigger = false
      if (player.getRole() === 'player' && (game as any).config?.buDao3TriggerHandler) {
        trigger = await (game as any).config.buDao3TriggerHandler({ playerId: player.getId() })
      ...
      if (trigger) {
        ...
        // 不立刻加入手牌 — 等分配决定后, 把留下的加入手牌, 给出去的 1 张直接给目标 (避免 UI 期间手牌多 3 张)
        eventBus.emit({
          type: 'card:draw',
        ...
        if (player.getRole() === 'player' && (game as any).config?.buDao3GiveHandler) {
          // 候选: 全部存活角色 (含自己 — 玩家可选择"全部留给自己"通过点自己或点取消)
          give = await (game as any).config.buDao3GiveHandler({
            zhangSanFengId: player.getId(),
            candidateIds: game.getAlivePlayers().map((p: any) => p.getId()),
            drawn,
          })
        } else if (player.getRole() !== 'player') {
          // AI: 给血量最低的队友 (玩家/友军优先), 不给自己 (默认留着)
        ...
        if (give) {
          ...
          const givenCard = drawn.find((c: any) => c.id === give!.cardId)
          const kept = drawn.filter((c: any) => c.id !== give!.cardId)
          // 留下的先加入手牌 (会触发 snapshot 让 UI 看到 2 张新牌)
          player.drawCards(kept)
          if (target && target.isAlive() && givenCard) {
            target.drawCards([givenCard])
            eventBus.emit({
              type: 'card:gain',
              sourceHeroId: target.getId(),
              data: { count: 1, reason: '布道', from: player.getId(), cards: [givenCard.id] },
            })
            game.emitSkillTrigger(player, '布道', `给${target.getName()} 1张${givenCard.name}`)
          }
        } else {
          // 不给: 3 张全部留给自己
      ...
      // 未发动 → fall through 走正常摸牌流程 (兵仙仍可叠加)
    }
