...
export function HandCard({ card, disabled, canPlayKill, isFullHp, aoJianActive, hasHongZhuang, isResponse, isJudgeReplace, isPending, isLifted, treasureSelectMode, selectDualMode, selectDiscardMode, hasValidSchemeTarget = true, huiChunAvailable, shadowed = false, hasLeiInJudge = false, onPlayKill, onPlayHeal, onEquip, onPlayScheme, onPlaySchemeSelf, onJudgeReplace, onRespondWithCard, onHuiChunHeal }: Props) {
  ...
  // 字号自适应: 长名字缩小
  const mainFontSize = card.name.length >= 3 ? 22 : 32
  const waterFontSize = card.name.length >= 3 ? 50 : 80
  ...
  return (
    <div
      ...
      style={{
        ...
        borderRadius: '6px',
        boxShadow: '0 4px 8px rgba(0,0,0,0.5), inset 0 0 12px rgba(139,105,20,0.2)',
    ...
    >
      {/* 双层装饰边框 (实线+虚线) */}
      <div style={{ position: 'absolute', inset: '3px', border: `1px solid ${theme.corner}`, borderRadius: '4px', pointerEvents: 'none' }} />
      <div style={{ position: 'absolute', inset: '5px', border: `1px dashed ${theme.corner}`, borderRadius: '3px', opacity: 0.55, pointerEvents: 'none' }} />
      ...
      {/* 角落花色+数字 (左上) */}
      <div style={{ position: 'absolute', top: '5px', left: '5px', display: 'flex', flexDirection: 'column', alignItems: 'center', lineHeight: 1, color: suitFontColor(card.suit) }}>
        <span style={{ fontSize: '11px', fontWeight: 'bold' }}>{suitSymbol[card.suit]}</span>
        <span style={{ fontSize: '13px', fontWeight: 'bold' }}>{num}</span>
      </div>

      {/* 角落花色+数字 (右下, 旋转180) */}
      <div style={{ position: 'absolute', bottom: '5px', right: '5px', display: 'flex', flexDirection: 'column', alignItems: 'center', lineHeight: 1, color: suitFontColor(card.suit), transform: 'rotate(180deg)' }}>
        <span style={{ fontSize: '11px', fontWeight: 'bold' }}>{suitSymbol[card.suit]}</span>
        <span style={{ fontSize: '13px', fontWeight: 'bold' }}>{num}</span>
      </div>
      ...
      {/* 主字 (底部) */}
      <div style={{
        position: 'absolute', bottom: card.name.length >= 3 ? '10px' : '14px', left: 0, right: 0,
        textAlign: 'center',
        color: isKill && effectiveIsRed ? '#c62828' : theme.main,
        fontSize: mainFontSize, fontWeight: 'bold', lineHeight: 1,
        textShadow: isKill && effectiveIsRed ? '0 0 6px rgba(198,40,40,0.4)' : 'none',
      }}>{mainChar}</div>
...
function CardIcon({ card, themeMain }: { card: Card; themeMain: string }) {
  ...
  if (card.type === 'equipment') {
    // 装备/弓
    return (
      <svg width="36" height="32" viewBox="0 0 32 32" fill={color}>
